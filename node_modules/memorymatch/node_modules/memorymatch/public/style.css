body {
    font-family: Arial, sans-serif;
    background-color: rgb(163, 157, 157);
    margin: 0;
    display: flex;
    justify-content: center;
    align-items: center;
    height: 100vh;
}

.game-container {
    text-align: center;
}

/* Blinking animation for game title */
#game-title {
    font-size: 2.5em;
    margin-bottom: 20px;
    animation: blinkColors 1.5s infinite;
}

@keyframes blinkColors {
    0% { color: red; }
    33% { color: blue; }
    66% { color: green; }
    100% { color: red; }
}

.card-grid {
    display: grid;
    grid-template-columns: repeat(6, 80px);
    grid-template-rows: repeat(6, 80px);
    gap: 10px;
    margin: 30px auto;
}

.card {
    width: 87px;
    height: 87px;
    background-color: #dce46ef1;
    cursor: pointer;
    display: flex;
    align-items: center;
    justify-content: center;
    font-size: 20px;
    color: white;
    position: relative;
    border-radius: 8px;
    transition: transform 0.3s;
}

.card img {
    width: 100%;
    height: 100%;
    display: none;
    border-radius: 10px;
}

.card.flipped img {
    display: block;
}

.card.flipped {
    background-color: rgb(178, 227, 114);
    border: 2px solid #4CAF50;
}

#reset-button {
    margin-top: 20px;
    padding: 10px 20px;
    font-size: 16px;
    cursor: pointer;
    border: none;
    background-color: #4CAF50;
    color: white;
    border-radius: 5px;
    transition: background-color 0.3s;
}

#reset-button:hover {
    background-color: #45a049;
}

/* Media Queries */

/* For tablets and smaller desktops (1024px and down) */
@media (max-width: 1024px) {
    #game-title {
        font-size: 2.2em;
    }

    .card-grid {
        grid-template-columns: repeat(4, 80px);
        grid-template-rows: repeat(6, 80px);
    }

    .card {
        width: 65px;
        height: 65px;
    }

    #reset-button {
        font-size: 14px;
    }
}

/* For small tablets and large phones (768px and down) */
@media (max-width: 768px) {
    #game-title {
        font-size: 1.8em;
    }

    .card-grid {
        grid-template-columns: repeat(3, 70px);
        grid-template-rows: repeat(6, 70px);
        gap: 8px;
    }

    .card {
        width: 60px;
        height: 60px;
        font-size: 16px;
    }

    #reset-button {
        font-size: 13px;
        padding: 8px 16px;
    }
}

/* For small phones (480px and down) */
@media (max-width: 480px) {
    #game-title {
        font-size: 1.5em;
    }

    .card-grid {
        grid-template-columns: repeat(3, 60px);
        grid-template-rows: repeat(6, 60px);
        gap: 5px;
    }

    .card {
        width: 55px;
        height: 55px;
        font-size: 14px;
    }

    #reset-button {
        font-size: 12px;
        padding: 6px 12px;
    }
}

/* For devices with 390px width (e.g., iPhone XR) */
@media (max-width: 390px) {
    #game-title {
        font-size: 1.4em;
    }

    .card-grid {
        grid-template-columns: repeat(3, 55px);
        grid-template-rows: repeat(6, 55px);
        gap: 5px;
    }

    .card {
        width: 50px;
        height: 50px;
        font-size: 12px;
    }

    #reset-button {
        font-size: 11px;
        padding: 5px 10px;
    }
}

/* For devices with 375px width (e.g., iPhone 11) */
@media (max-width: 375px) {
    #game-title {
        font-size: 1.3em;
    }

    .card-grid {
        grid-template-columns: repeat(3, 50px);
        grid-template-rows: repeat(6, 50px);
        gap: 5px;
    }

    .card {
        width: 48px;
        height: 48px;
        font-size: 11px;
    }

    #reset-button {
        font-size: 10px;
        padding: 5px 9px;
    }
}

/* For very small phones (360px and down) */
@media (max-width: 360px) {
    #game-title {
        font-size: 1.2em;
    }

    .card-grid {
        grid-template-columns: repeat(2, 50px);
        grid-template-rows: repeat(6, 50px);
        gap: 4px;
    }

    .card {
        width: 45px;
        height: 45px;
        font-size: 10px;
    }

    #reset-button {
        font-size: 9px;
        padding: 4px 8px;
    }
}
